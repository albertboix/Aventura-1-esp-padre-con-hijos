...
class Logger {
    constructor(prefix = '') {
        ...
        this.correlacionId = this.generarIdUnico();
    }
    ...
    setNivel(nivel) {
        if (typeof nivel === 'string') {
            nivel = LOG_LEVELS[nivel.toUpperCase()] || LOG_LEVELS.INFO;
        }
        config.logLevel = nivel;
    }
        ...
        if (datos) {
            if (datos instanceof Error) {
                contexto.error = {
                    ...
                    stack: config.debug ? datos.stack : undefined,
                ...
                contexto.detalles = String(datos);
            }
        }
    ...
    log(nivel, mensaje, datos) {
        const logEntry = this.formatearMensaje(nivel, mensaje, datos);
        
        switch(nivel.toLowerCase()) {
            case 'debug':
                if (config.logLevel <= LOG_LEVELS.DEBUG) {
                    console.debug(JSON.stringify(logEntry, null, config.debug ? 2 : 0));
                }
                break;
            case 'info':
                if (config.logLevel <= LOG_LEVELS.INFO) {
                    console.info(JSON.stringify(logEntry, null, config.debug ? 2 : 0));
                }
                break;
            case 'warn':
                if (config.logLevel <= LOG_LEVELS.WARN) {
                    console.warn(JSON.stringify(logEntry, null, config.debug ? 2 : 0));
                }
                break;
            case 'error':
                if (config.logLevel <= LOG_LEVELS.ERROR) {
                    console.error(JSON.stringify(logEntry, null, config.debug ? 2 : 0));
                }
                break;
            default:
                console.log(JSON.stringify(logEntry, null, config.debug ? 2 : 0));
        }
    ...
    debug(mensaje, datos) {
        return this.log('debug', mensaje, datos);
    }

    info(mensaje, datos) {
        return this.log('info', mensaje, datos);
    }

    warn(mensaje, datos) {
        return this.log('warn', mensaje, datos);
    }

    error(mensaje, datos) {
        return this.log('error', mensaje, datos);
    }

    crearHijo(sufijo) {
        const nuevoPrefijo = this.prefix ? `${this.prefix}:${sufijo}` : sufijo;
        const loggerHijo = new Logger(nuevoPrefijo);
        return loggerHijo;
    }
}
...
const mensajeria = {
    ...
    // Métodos de utilidad para uso sin inicialización previa
    generarId: generarIdUnico,
...
// Hacer que la API esté disponible globalmente (opcional)
if (typeof window !== 'undefined' && !window.mensajeria) {
    window.mensajeria = mensajeria;
}

// Exportar para módulos (CommonJS, AMD, ES modules)
 ...
 */
async function manejarMensajeSistema(accion, datos = {}, contexto = {}) {
    const { mensajeId, origen, timestamp } = contexto;
